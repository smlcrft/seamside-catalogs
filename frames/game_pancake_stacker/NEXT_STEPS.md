...
- **Multiplayer race** — scores are already the space's `pancake_scores` table; let multiple viewers stack against each other on adjacent griddles in real time. Top of leaderboard wins the round.
- **Skins** — let the space's owner pick a theme (pancakes / waffles / dosas / blini), kept as the session's own setting.
