...
- **History sparkline** — keep the last 24 hours of indicator scores (in memory, or a small table of the space) and render a tiny sparkline above each spectrum to show the trend, not just the snapshot.
...
- **Daily journal hook** — log each settings save and indicator snapshot as a per-day row of a table in the space the gardener can scroll back through, useful when troubleshooting why a crop struggled.
