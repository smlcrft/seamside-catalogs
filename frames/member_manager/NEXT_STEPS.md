...
The frame is intentionally lean — a `members` list of the space for the roster (bound per session), the session's own preferences, and an inline editor. Ideas to extend it:
...
2. **Sort options** — toggle between sort-by-name, sort-by-role, and sort-by-date-added; persist the choice in the settings.
...
6. **Activity log** — declare a second table (`members_activity`) that records role changes / additions / removals so peers can see history.
