  ...
  "items": [
    {
      ...
      "tags":            ["household", "garden"]         // OPTIONAL — catalog tags for browsing/grouping (omitted when unset)
    }
  ]
}
```
...
- **`tags`** (frame.json only) — an optional array of catalog tags for browsing and grouping, copied verbatim into the frame's manifest item (omitted when empty). Purely descriptive: no install or permission behavior. The build **validates**: at most **8** unique tags, each a lowercase kebab slug (`^[a-z0-9][a-z0-9-]*$`) of at most **24** chars. Two kinds of tag, freely mixed:
  - **generic topical tags** — broad audiences/topics shared across the library, e.g. `everyday`, `household`, `study`, `smallbiz`, `homelab`, `play`, `studio`, `assist`, `finance`, `games`, `weather`, `community`.
  - **specific set tags** — a tag shared by a small set of directly related frames (companions over a shared table, or variants of one schema), e.g. `garden` on the garden planner and its companions. This is the lightweight precursor to a future set/grouping identifier; until that exists, a shared specific tag *is* how related frames find each other in the catalog.
